...
class KNN:
    def __init__(self, k, metric="cosine"):
        ...
        self.k = k
        self.metric = metric
        self.cls = KNeighborsClassifier(self.k, metric=self.metric)
    ...
    def knn(self, features, labels):
        ...
        with torch.no_grad():
            ...
            # fit
            self.cls.fit(features_np, labels_np)
...
class KMeansClustering:
    def __init__(self, n_clusters):
        ...
        self.clusterer = KMeans(n_clusters=self.n_clusters, init="k-means++", max_iter=200)

    def fit(self, features):
        """
        Fit the KMeans model on the input features.
        Args:
            features: [... , dataset_size, feat_dim]
        """
        feature_dim = features.shape[-1]
        features_np = features.cpu().view(-1, feature_dim).detach().numpy()
        self.clusterer.fit(features_np)
        labels = self.clusterer.predict(features_np)
